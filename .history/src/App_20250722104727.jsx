...
function Scene({ onPositionsReady, hasClickedMonitor, monitorOpacity }) {
	const { scene } = useGLTF(
		import.meta.env.BASE_URL + "kenny_portfolio_copy (13).glb"
	);
	const monitorScreenRef = useRef();
	const texture_monitor = useLoader(
		TextureLoader,
		import.meta.env.BASE_URL + "portfolio2.png"
	);
	...
	const texture_resume = useLoader(
		TextureLoader,
		import.meta.env.BASE_URL + "resume12.png"
	);
	...
	const texture_tablet = useLoader(
		TextureLoader,
		import.meta.env.BASE_URL + "ipad.png"
	);
	...
	useEffect(() => {
		if (monitorScreenRef.current) {
			monitorScreenRef.current.material.opacity = parseFloat(monitorOpacity);
