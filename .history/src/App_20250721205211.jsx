...
export default function App() {
	...
	return (
		<>
			...
			>
				<ambientLight color={0x404040} intensity={0.9} />
				<mesh ref={lightTarget} position={[0, 15, -12]} visible={false} />

				{/*spot Light Shining from the back*/}
				<spotLight
					position={[0, 40, 0]}
					color={0xffffff} //brownish color
					intensity={2}
					angle={0.5}
				...
				{/* back wall */}
				<mesh position={[0, 10, 72]} rotation={[0, Math.PI, 0]} receiveShadow>
					<planeGeometry args={[1100, 90]} />
					<meshStandardMaterial color={wall_colour} /> {/*0x prefix*/}
				</mesh>
				...
				>
					...
					<meshStandardMaterial color={wall_colour} side={THREE.DoubleSide} />
				</mesh>
				...
				{positions?.monitor && !hasClickedMonitor && (
					<mesh
						...
						onClick={() => {
							goTo("monitor");
							setHasClickedMonitor(true);
							setShowMonitorIframe(true);
						}}
					>
				...
				{positions?.clipboard && (
					<mesh
						...
						onClick={() => {
							...
							// optionally set some state like setHasClickedClipboard(true)
						}}
					>
